...
/* Método de DFP -- Atualiza a matriz hessiana no método de Quase-Newton */
std::valarray< std::valarray<double> > dfp(std::valarray< std::valarray<double> > h, std::valarray<double> p, std::valarray<double> q)
{
	//Utiliza o metodo DFP para calcular a aproximacao da inversa da hessiana
	double denominador = (multVetMatriz(q, h)*q).sum();
	std::valarray< std::valarray<double> > parcela1 = divMatriz(multVetor(p, p), (p*q).sum());
	std::valarray< std::valarray<double> > parcela2 = divMatriz(multMatriz(multVetor(multMatVet(h, q), q), h), denominador);

	return subMatriz(somaMatriz(h, parcela1), parcela2);
}

/* Método de Quase-Newton */
std::valarray<double> qNewton(std::valarray<double> x, double rho = 1.0, bool useDFP=false, int maxIter = 1000)
{
	...
	while (norma(x-x0) != 0 && nIter < maxIter){
		gf = grad(x, rho, true);
	...
	std::cout << "Iter. qNewton: " << nIter << std::endl;
...
std::valarray<double> penalidade(std::valarray<double> x, double rho=1.0, double beta=3, double epslon=10e-6, int maxIter = 100)
{
	double initx[] = {1, 1, 1};
	...
	while(norma(x - x0) > epslon && nIter < maxIter)
	{
		x0 = x;
		x = qNewton(holder, rho, true);
		rho = rho*beta;   
		nIter++;
	}
